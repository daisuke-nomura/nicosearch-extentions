...
function loadJSON(str, cb) {
	...
	opera.extension.postMessage(str);
}
...
function show_suggest(res, id, suggest, buffArray, input) {
  ...
  for (var i = 0, c = res.length; i < c; i++) {
    ...
    img.src = favicon;//chrome.extension.getURL("favicon.png");
    img.align = "left";
	...
	if(document.URL.host == "live.nicovideo.jp" || document.URL.host == "seiga.nicovideo.jp") {	// ホストがlive.nicovideo.jpか否かで振り分け
